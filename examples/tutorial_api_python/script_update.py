...
params["default_model_folder"] = "/home/nikhiltokala1/openpose/models/"
# Construct OpenPose object allocates GPU memory
openpose = OpenPose(params)

# Read new image
img = cv2.imread("/home/nikhiltokala1/openpose/examples/media/rsz_img_8894.jpg")
...
def distance_from_range(value, bound1, bound2):
    if (bound1 <= value <= bound2):
        ...
        return min(value-bound1, value-bound2)
...
def neckfeedback(deviation):
    if (deviation == 0):
        return "Your neck posture is great! Remember to keep this position while working."
    elif (0 < deviation < 10):
        return "Consider changing your neck posture slightly. Adjusting your neck forwards is recommended."
    elif (-10 <deviation < 0):
        return "Consider changing your neck posture slightly. Adjusting your neck backwards is recommended."
    elif (deviation>=10):
        return "It is highly recommended to correct your neck posture. In addition to moving your neck forwards, try lowering your monitor so your neck is straighter."
    else:
        return "It is highly recommended to correct your neck posture. In addition to moving your neck backwards, try raising your monitor so your neck is straighter."

#for foot angle, look to see if calfthigh>90 or <90 to decide direction of foot
output = np.array([neckfeedback(distance_from_range(neckback, 130, 150)), distance_from_range(backrthigh, 85, 95), distance_from_range(rthighrcalf, 80, 105), distance_from_range(lthighlcalf, 80, 105), distance_from_range(lfootlthigh, 170, 180), distance_from_range(rfootrthigh, 170, 180)])
np.savetxt("output5.txt", output, fmt="%s")
